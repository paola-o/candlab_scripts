...
#Relative mean: zero out the 9th row (row #8 if counting in python) (because motion is relative to the previous timepoint)
    ...
    if [ -e $movementDir2/"$sub"_"$run"_outliers* ]; then
      filename="$(ls ""$movementDir2"/"$sub"_"$run"_outliers"*)"
    ...
    printf "\t$sub \t "$run" \t $relmean2 \t $absmean2 \t $fdmean2 \t $percentfd2Over2 \t $percentfd2Over5 \t $stdev2 \t $lowrange \t $highrange \t $outliers \t $percoutliers \t $threshold" >> "$datadir"/ses-scerV1_task-"$run"_bold/Motionstats_summary.csv



    if [[ "$run" == "${block[@]:0:1}" ]]; then
    #block=("sc1part1" "sc1part2" "sc2part1" "sc2part2")
		  printf "\t$sub \t "$run" \t $relmean2 \t $absmean2 \t $fdmean2 \t $percentfd2Over2 \t $percentfd2Over5 \t $stdev2 \t $lowrange \t $highrange \t $outliers \t $percoutliers \t $threshold" >> /gpfs/milgram/project/gee_dylan/candlab/scripts/scer/mri/motion_stats/Allsubjects_movementstatistics.csv
      printf "\t$sub \t "$run" \t $relmean2 \t $absmean2 \t $fdmean2 \t $percentfd2Over2 \t $percentfd2Over5 \t $stdev2 \t $lowrange \t $highrange \t $outliers \t $percoutliers \t $threshold" >> "$datadir"/Motionstats_summary_allruns.csv
      printf "\t$sub \t "$run" \t $relmean2 \t $absmean2 \t $fdmean2 \t $percentfd2Over2 \t $percentfd2Over5 \t $stdev2 \t $lowrange \t $highrange \t $outliers \t $percoutliers \t $threshold" >> "$outdir"/"$sub"_motionstats_summary.csv
